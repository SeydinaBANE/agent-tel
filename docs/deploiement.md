...
| Production | Appels réels | Railway / EC2 / VPS |
...
cp .env.example .env
# Éditer .env : OPENROUTER_API_KEY, TWILIO_*, PUBLIC_URL
...
Railway gère nativement Python, WebSocket, et les variables d'environnement. Pas besoin de configurer TLS manuellement.
...
# 2. Le Procfile est déjà présent dans le repo :
...
# PUBLIC_URL sera ajouté après le premier déploiement
...
> **Note ffmpeg sur Railway** : Railway ne fournit pas ffmpeg par défaut.
> Utiliser le Dockerfile (Railway détecte automatiquement un Dockerfile) ou ajouter un `nixpacks.toml` :
>
> ```toml
> # nixpacks.toml
> [phases.setup]
...
- [ ] Modèle Whisper adapté : `small` ou `medium` recommandé en prod
- [ ] Validation signature Twilio (`X-Twilio-Signature`) activée
- [ ] Variables d'environnement dans un secrets manager (pas dans `.env` git)
...
| Voix TTS trop lente | edge-tts latence réseau | Vérifier la connectivité, envisager ElevenLabs |
